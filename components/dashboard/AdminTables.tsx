...
export function UserManagementTable({ items, mutate }: { items: AdminItem[]; mutate: Mutate }) {
  ...
  return <SelectableTable items={items} itemLabel="user" onDelete={(ids) => mutate("/api/admin/users", "DELETE", { action: "bulk_delete", userIds: ids }, `${ids.length} users deleted`)} headers={["User", "Role", "Status", "Joined", "Actions"]} render={(item) => <>
    <td><b style={{ color: "var(--p-ink, #0f172a)" }}>{String(item.name)}{Boolean(item.isSuperAdmin) && " · Super Admin"}</b><small style={{ color: "#64748b" }}>{String(item.email)}</small></td>
    <td><select disabled={Boolean(item.isSuperAdmin)} defaultValue={String(item.role)} id={`role-${item.id}`} style={{ background: "#ffffff", color: "#0f172a", border: "1px solid #cbd5e1", padding: "6px 10px", borderRadius: "8px", fontWeight: 700, fontSize: "12px" }}><option value="admin">Admin</option><option value="store_owner">Shop owner</option><option value="customer">Customer</option></select></td>
    <td><select disabled={Boolean(item.isSuperAdmin)} defaultValue={String(item.status)} id={`status-${item.id}`} style={{ background: "#ffffff", color: "#0f172a", border: "1px solid #cbd5e1", padding: "6px 10px", borderRadius: "8px", fontWeight: 700, fontSize: "12px" }}><option value="active">Active</option><option value="suspended">Suspended</option><option value="disabled">Disabled</option><option value="banned">Banned</option></select></td>
    <td style={{ color: "#475569", fontWeight: 600 }}>{new Date(Number(item.createdAt) * 1000).toLocaleDateString()}</td>
    <td><div className="tableActions" style={{ display: "flex", gap: "6px", alignItems: "center" }}>{item.role !== "admin" && <button type="button" onClick={() => void startChat(item)} style={{ background: "#f1f5f9", color: "#0f172a", border: "1px solid #cbd5e1", padding: "6px 12px", borderRadius: "8px", fontWeight: 700, cursor: "pointer" }}>Chat</button>}<button disabled={Boolean(item.isSuperAdmin)} onClick={() => void mutate("/api/admin/users", "PATCH", { userId: item.id, role: (document.getElementById(`role-${item.id}`) as HTMLSelectElement).value, status: (document.getElementById(`status-${item.id}`) as HTMLSelectElement).value }, "User updated")} style={{ background: "linear-gradient(135deg, #FF5722 0%, #E53935 100%)", color: "#ffffff", border: "none", padding: "6px 14px", borderRadius: "8px", fontWeight: 700, cursor: "pointer" }}>Save</button><button disabled={Boolean(item.isSuperAdmin)} onClick={() => { if (window.confirm(`Delete ${String(item.email)}?`)) void mutate("/api/admin/users", "DELETE", { userId: item.id }, "User deleted"); }} style={{ background: "#ef4444", color: "#ffffff", border: "none", padding: "6px 12px", borderRadius: "8px", fontWeight: 700, cursor: "pointer" }}>Delete</button></div></td>
  </>} />;
}

...
export function OwnerHealthcarePanel({ storeId }: { storeId: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const result = await apiFetch<Data>(`/api/owner/healthcare?storeId=${encodeURIComponent(storeId)}`);
      startTransition(() => {
        setData(result);
        setError("");
      });
  ...
  const configure = useCallback((event: FormEvent<HTMLFormElement>) => {
  ...
  const { profile, entries, waiting, called, nextPatient, activationStatus, queueEligible } = useMemo(() => {
    const prof = data.profile ?? {};
    const ent = data.entries ?? [];
    const w = ent.filter((entry) => entry.status === "waiting");
    const c = ent.find((entry) => entry.status === "called");
  ...
  const renderedQueueTable = useMemo(() => {
    return entries.map((entry) => (
      <article key={String(entry.id)} className={entry.status === "called" ? "active" : ""}>
        <b>#{entry.tokenNumber}</b>
        <span>
          <strong>{entry.isEmergency ? "Emergency · " : entry.isWalkIn ? "Walk-in · " : ""}{entry.patientName}</strong>
          <small>{entry.status} · {String(entry.arrivalStatus ?? "waiting").replaceAll("_", " ")} · {new Date(Number(entry.joinedAt) * 1000).toLocaleTimeString()}</small>
        </span>
        {entry.status === "waiting" && (
          <div className="tableActions">
            <button onClick={() => void action("skip", { entryId: entry.id })}>Skip</button>
            <button className="dangerButton" onClick={() => void action("remove", { entryId: entry.id })}>Remove</button>
          </div>
        )}
      </article>
    ));
  ...
  const renderedHistory = useMemo(() => {
    return (data.history ?? []).slice(0, 7).map((day) => (
      <div key={String(day.serviceDate)}>
        <span>{day.serviceDate}</span>
        <b>{day.completed}/{day.total} completed</b>
        <small>{day.skipped} skipped</small>
      </div>
    ));
  }, [data.history]);

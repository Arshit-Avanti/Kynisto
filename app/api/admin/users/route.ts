    ...
    if (body.action === "bulk_delete") {
      ...
      for (const id of userIds) {
        await db.batch([
          ...
          db.prepare("DELETE FROM healthcare_queue_entries WHERE user_id = ?").bind(id),
          db.prepare("DELETE FROM conversation_participants WHERE user_id = ?").bind(id),
...
export async function DELETE(request: Request) {
  try {
    ...
    await db.batch([
      ...
      db.prepare("DELETE FROM healthcare_queue_entries WHERE user_id = ?").bind(userId),

...
export default async function HealthcareDashboardLayout({ children }: { children: React.ReactNode }) {
  const user = await requirePageRole(["store_owner", "admin"], "/healthcare/dashboard");
  return <PortalShell user={user} workspaceRole="healthcare_owner">{children}</PortalShell>;
}
